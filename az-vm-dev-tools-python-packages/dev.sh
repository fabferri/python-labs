...
# check if the xrdp service is active
for i in {1..5}
do
   STATUS="$(systemctl is-active xrdp.service)"
   if [[ $STATUS == 'active' ]]
   then
      break
   fi
...
userFolder=$(awk -v FS=':' -v user="$admUser" '($1==user) {print $6}' "/etc/passwd")


#Downloading the latest Miniconda installer for Linux. Your architecture may vary.
wget https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh -O /tmp/miniconda.sh
# silent installation of Miniconda in user folder:
# -b: Batch mode 
# -p: Installation prefix/path.
su $admUser -c "bash /tmp/miniconda.sh -b -p $userFolder/miniconda"


# After installing, initialize your newly-installed Miniconda
# install conda’s shell functions for easier access in the future
su $admUser -c "$userFolder/miniconda/bin/conda init bash"
